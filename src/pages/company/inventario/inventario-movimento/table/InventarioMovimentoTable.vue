      ...
      <div class="card-body">
        ...
        <div class="row">
          <div class="col-12 col-md-6 form-group" v-if="!invisibleFields['articolo']">
            <template >
              <input-autocomplete v-model="entity['articolo']" :config="configTypes['articolo']"
                :readonlyAttr="readonlyFields['articolo']" label="Articolo"
                fieldName="articolo">
              </input-autocomplete>
            </template>
          </div>
          <div class="col-12 col-md-6 form-group" v-if="!invisibleFields['dataRegistrazione']">
            <input-date-range v-model="entity['dataRegistrazione']"
                :readonlyAttr="readonlyFields['dataRegistrazione']" label="Data registrazione"
          ...
          <div class="col-12 col-md-6 form-group" v-if="!invisibleFields['causale']">
            <input-select v-model="entity['causale']"
                :items="this.domini['CAUSALE_MOVIMENTO_INVENTARIO']"
                :readonlyAttr="readonlyFields['causale']" label="Causale"
                fieldName="causale">
            </input-select>
          </div>
          <div class="col-12 col-md-6 form-group" v-if="!invisibleFields['dataScadenza']">
            <input-date-range v-model="entity['dataScadenza']"
                :readonlyAttr="readonlyFields['dataScadenza']" label="Data scadenza"
...
export default {
  data() {
    return {
      ...
      propsColumns: { articolo: { bind: 'articolo.codice', type: 'autocomplete' },
      ...
      configTypes: { articolo: { isDominio: false, urlApi: '/api/prodottos/all', fieldId: '_id', showCodice: true } },
      dominiToLoad: { segno: 'SEGNO_MOVIMENTO_INVENTARIO', causale: 'CAUSALE_MOVIMENTO_INVENTARIO' },
      domini: [],
      propsFilterEntity: { articolo: { bind: 'articolo', type: 'equals' }, dataRegistrazione: { bind: 'dataRegistrazione', type: 'range' }, segno: { bind: 'segno', type: 'equals' }, causale: { bind: 'causale', type: 'equals' }, dataScadenza: { bind: 'dataScadenza', type: 'range' } },
    ...
    getEntities(page, rowsPerPage) {
      ...
      if (this.entity) {
        ...
        const filterDateArray = Object.keys(this.entity)
          .filter(keyFilter => this.entity[keyFilter] && this.propsFilterEntity[keyFilter].type === 'range')
          .map(keyFilter => `${keyFilter}.greaterThanEqual=${this.entity[keyFilter][0]}&${keyFilter}.lessThanEqual=${this.entity[keyFilter][1]}`);
